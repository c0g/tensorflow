...
class MatrixTriangularSolveOpGPU : public LinearAlgebraOp<Scalar> {
 public:
  ...
  void ComputeMatrix(OpKernelContext* context, const ConstMatrixMaps& inputs,
    ...
    uint64 lda = matrix.cols();    // a_in_matrix_shape.dim_size(1);
    uint64 ldb = rhs.cols();       // b_in_matrix_shape.dim_size(1);
    uint64 cublas_m = rhs.cols();  // b_in_matrix_shape.dim_size(1);
    uint64 cublas_n = rhs.rows();  // b_in_matrix_shape.dim_size(0);
